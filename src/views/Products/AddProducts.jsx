...
import { getHSN, addProduct, getProducts } from "../../actions/products";
...
class AddProducts extends Component {
  constructor(props) {
    super(props);
    let data = this.props.location && this.props.location.state;
    this.state = {
      redirected: data && data.redirected,
      searched_data: data && data.keyword,
      barcoded: data && data.barcoded,
      db: new database(),
      product_type: 1,
      product_name: (!(data && data.barcoded) ? (data && data.keyword) : ""),
      variants: [
        {
          name: "",
          barcode: data && data.barcoded ? data && data.keyword : "",
          price_stock: [
            {
              cost_price: "",
              id: "",
  ...
  async UNSAFE_componentWillReceiveProps(newProps) {
    if (newProps.hsn_data && newProps.hsn_data.code === 200 && hsn_flag) {
      this.setState({
    ...
    if (
      ...
      if (this.state.redirected === "product_page") {
        this.props.history.push({
          pathname: "/products",
          state: { keyword: this.state.searched_data }
        });
      } else if (this.state.redirected === "billing_page") {
        this.props.history.push({
          pathname: "/billing",
          state: {
            keyword: this.state.searched_data,
            barcoded: this.state.barcoded
          }
        });
      } else {
    ...
    if (
      newProps.add_response.code === 200 &&
      add_flag
    ) {
      toaster("success", newProps.add_response.message);
      ...
      newProps.add_response &&
      newProps.add_response.code === 400 &&
      add_flag
    ) {
      toaster("error", newProps.add_response.message);
      add_flag = false;
    }
  }

  handleChange = (e, name, index, stock_index) => {
    ...
    if (
      name === "product_name" && /^[0-9A-Z a-z_-]{0,250}$/.test(e.target.value) === false
    ) {
      return;
    } else if (
      name === "name" &&
      ...
      name === "product_description" && e.target.value.length > 500
    ) {
      return;
    } else if (
      name === "search_keyword" &&
      ...
      let variants = this.state.variants;
      variants[index].barcode = e.target.value;
      this.setState({ variants: variants });
    } else if (name === "quantity") {
      let variants = this.state.variants;
      variants[index].price_stock[stock_index].quantity = e.target.value;
      this.setState({ variants: variants });
    } else if (name === "weight") {
      let variants = this.state.variants;
      variants[index].price_stock[stock_index].weight = e.target.value;
      this.setState({ variants: variants });
    } else if (name === "cost_price") {
      let variants = this.state.variants;
      variants[index].price_stock[stock_index].cost_price = e.target.value;
      this.setState({ variants: variants });
    } else if (name === "selling_price") {
      let variants = this.state.variants;
      variants[index].price_stock[stock_index].selling_price = e.target.value;
      this.setState({ variants: variants });
    } else if (name === "mrp") {
      let variants = this.state.variants;
      variants[index].price_stock[stock_index].mrp = e.target.value;
      this.setState({ variants: variants });
  ...
  handleImageChange = e => {
    ...
    this.setState({ show_image: URL.createObjectURL(e.target.files[0]), image_url: e.target.files[0] });
  };
  ...
  submitData = () => {
    ...
    variants.map((data, key) => {
      return (
        data.price_stock.map(inner_data => {
    ...
    if (parseInt(product_type) === 2 && weight.some(
      item => item === 0
    )) {
    ...
    if (navigator.onLine) {
      ...
      toaster(
        "warning",
        "No Internet. You wouldn’t be able to add new product without internet"
      );
    }
  };

  removeImage = () => {
    this.setState({ image_url: "", show_image: '' });
  };
                    ...
                    <div className="row">
                      <div className="form-group col-12 col-sm-6">
                        ...
                        <select
                          ...
                          value={product_type}
                          onChange={e => this.handleChange(e, "product_type")}
                        >
                          {product_types.map((option, key) => (
                                                ...
                                                <tr>
                                                  {product_type === "2" && (
                                                    <td>
                                                      ...
                                                      />
                                                      <small>(Add weight in grams)</small>
                                                    </td>
                                                  )}
                                                  {product_type !== "3" && (
                                                    <td>
                  ...
                  <div className="col-12 col-md-11 mx-auto">
                    <div className="row">
                            ...
                            <div className="form-group col-12 col-md-6">
                              ...
                              <div className="input-group">
                                <input
                                  ...
                                  name=""
                                  value={hsn_code}
                                  // onChange={e => this.callHSNHelper(e)}
                                  onChange={e =>
                      ...
                      <div className="form-group col-12">
                        <label>PRODUCT DESCRIPTION <small>(upto 500 characters)</small></label>
                        <textarea
                          className="form-control"
                        ...
                        <div className="card p-3">
                          {show_image ? (
                            <div className="viewUpload-img">
                              <img src={show_image} alt="products" />
                              <button
                                type="button"
                                className="close"
...
const mapStateToProps = store => {
  return {
    ...
    add_response: store.products.add_response
  };
};
